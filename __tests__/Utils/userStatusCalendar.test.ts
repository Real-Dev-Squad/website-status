...
describe('Unit | Utils | getStartOfDay', () => {
    test('returns the start of the day for a valid date input', () => {
    ...
    test('returns the start of the day for a date with middle time', () => {
        const middleTimeDate = new Date(2022, 11, 25, 12, 30, 0);
        const result = getStartOfDay(middleTimeDate);
        expect(result).toEqual(new Date(2022, 11, 25, 0, 0, 0));
    });

    test('returns the start of the day for a date with end of the day time', () => {
        const endOfDayDate = new Date(2022, 11, 25, 23, 59, 59);
        const result = getStartOfDay(endOfDayDate);
        expect(result).toEqual(new Date(2022, 11, 25, 0, 0, 0));
    });

    test('returns today’s date with start of the day for an invalid date input', () => {
...
describe('Unit | Utils | getDatesInRange', () => {
    test('returns an array of unix time of the start of each day between the date range.', () => {
    ...
    test('handles a single day range and has one element in the array.', () => {
        const startDate = new Date(2022, 11, 25);
    ...
    test('handles a day with one day range and has two unix timestamps in the array.', () => {
    ...
    test('handles dates with middle of the time, it returns an array with unix timestamps of dates between range including edge dates.', () => {
        const startDate = new Date(2022, 11, 25, 12, 0, 0);
    ...
    test('handles invalid range and should return an empty array.', () => {
        const startDate = new Date(2022, 11, 25);
...
describe('Unit | Utils | processData ', () => {
    test('returns empty objects for null itemId', () => {
        const result = processData(null, []);
        expect(result).toEqual([{}, {}]);
    });

    test('returns empty objects for itemId with no matching log', () => {
        const result = processData('nonexistentUserId', []);
        expect(result).toEqual([{}, {}]);
    });

    test('returns empty objects for log with no data', () => {
        const logWithNoData = [
            {
                userId: 'user1',
                data: [],
